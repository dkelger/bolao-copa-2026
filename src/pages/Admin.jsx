...
export default function Admin() {
  ...
  async function distribuirColocacaoFinal() {
    if (!colocacaoFinal.primeiro || !colocacaoFinal.segundo || !colocacaoFinal.terceiro) {
      setColocacaoMsg('❌ Selecione os 3 times!'); return
    }
    setSavingColocacao(true); setColocacaoMsg('')
    try {
      const times = [
        { teamId: colocacaoFinal.primeiro, pontos: 10, desc: '🥇 1º lugar final +10pts' },
        { teamId: colocacaoFinal.segundo, pontos: 6, desc: '🥈 2º lugar final +6pts' },
        { teamId: colocacaoFinal.terceiro, pontos: 3, desc: '🥉 3º lugar final +3pts' },
      ]
      const logs = []
      for (const t of times) {
        const { data: jaExiste } = await supabase.from('points_log').select('id')
        ...
        const { data: teamPicks } = await supabase.from('picks').select('user_id').eq('team_id', t.teamId)
        ;(teamPicks || []).forEach(p => {
          logs.push({ user_id: p.user_id, team_id: t.teamId, tipo: 'colocacao_final', pontos: t.pontos, descricao: t.desc, fase: 'final' })
        })
      }
      if (logs.length > 0) await supabase.from('points_log').insert(logs)
      setColocacaoMsg(`✅ Pontos distribuídos! ${logs.length} registros inseridos.`)
      await loadAll()
    } catch (err) {
      setColocacaoMsg('❌ Erro: ' + err.message)
    } finally {
